...
fn normalize_json_schema(schema: Value) -> Value {
    match schema {
        Value::Object(mut obj) => {
            ...
            // Keep reusable definitions and references intact, while applying
            // the same transport cleanup inside each definition. Anthropic
            // schemas generated by modern SDKs commonly place nested objects
            // under `$defs`; leaving their `$schema`/`$id` metadata untouched
            // produces an inconsistent Gemini JSON schema payload.
            for key in ["$defs", "definitions"] {
                if let Some(definitions) = obj.get_mut(key).and_then(|value| value.as_object_mut())
                {
...
mod tests {
    ...
    #[test]
    fn normalizes_transport_metadata_inside_schema_definitions() {
        let schema = json!({
            "$schema": "https://json-schema.org/draft/2020-12/schema",
            "type": "object",
        ...
        let result = build_gemini_function_declaration("lookup", None, schema);
        let normalized = &result["parametersJsonSchema"];
        assert!(normalized.get("$schema").is_none());
        assert!(normalized["$defs"]["item"].get("$schema").is_none());
        assert!(normalized["$defs"]["item"].get("$id").is_none());
